 ...
 */
function getParserForLanguage(languageName) {
 ...
 */
function isConsideredIdentifier(node, languageName) {
...
function isKeywordOrOperatorOrPunctuation(node, languageName) {
    ...
    if (type.endsWith('_keyword'))
        return true; // e.g., 'public_keyword', 'static_keyword', 'void_keyword'
...
function extractTokensFromNode(node, tokens, vocabulary, languageName) {
    ...
    if (node.type === 'string_literal' || node.type === 'string' || node.type.includes('string_fragment')) {
        ...
        const stringTokens = stringContent.split(/\s+/).filter((token) => token.trim().length > 0);
    ...
    // 3. Handle XML specific nodes (retaining and adapting current logic)
        ...
        const parts = textContent.split(/[<>\s]+/).filter((part) => part.trim().length > 0);
