...
export class AzureRateLimiter {
    ...
    async waitForPermit() {
        ...
        if (now < this.coolDownUntil) {
            ...
            console.log(`AzureRateLimiter: Cooling down for ${waitTime}ms.`);
    ...
    notifyRateLimit(retryAfterSeconds) {
        ...
        if (newCoolDownUntil > this.coolDownUntil) {
            ...
            console.log(`AzureRateLimiter: Rate limit hit. Cooling down until ${new Date(this.coolDownUntil).toISOString()}.`);
        }
    }
}
