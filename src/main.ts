...
async function main() {
  try {
    ...
    if (isNaN(vocabMinDf) || vocabMinDf < 1) throw new Error("VOCAB_MIN_DF must be a positive integer.");
    if (isNaN(vocabMaxDf) || vocabMaxDf <= 0 || vocabMaxDf > 1) throw new Error("VOCAB_MAX_DF must be a number between 0 and 1.");
    if (isNaN(vocabTargetSize) || vocabTargetSize < 1) throw new Error("VOCAB_TARGET_SIZE must be a positive integer.");
   ...
   const vocabChunker = new Chunker(
       analysisService, // Still needs analysisService instance, but analysis is skipped
   ...
   const vocabChunks: Chunk[] = Array.from(vocabFileChunksMap.values()).flat();
