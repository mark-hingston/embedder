...
export class StateManager {
  ...
  async loadState(): Promise<FilePointsState> {
    try {
      ...
      console.log(
        `Loaded state for ${Object.keys(state.files || {}).length} files (Commit: ${state.lastProcessedCommit || 'N/A'}) from ${STATE_FILE_NAME}`
      );
      ...
      if (!state.files && Object.keys(state).length > 0 && !state.lastProcessedCommit) {
        ...
        return {
          files: state as Record<string, string[]>, // Assume the old root object was the file map
      ...
      if (error instanceof Error && 'code' in error && error.code === 'ENOENT') {
        ...
        console.warn(
          `Warning: Could not read or parse ${STATE_FILE_NAME}: ${errorMessage}. Proceeding with empty state.`
        );
      }
  ...
  getPointsForFiles(files: Set<string>, currentState: FilePointsState): string[] {
    const pointIds = new Set<string>(); // Use a Set to automatically handle duplicates
